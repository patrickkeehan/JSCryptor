(function() {
  ...
  var _encrypt = function(plain_text, components, encryption_key, hmac_key) {
    let cipher = crypto.createCipheriv(_settings.algorithm, encryption_key, components.headers.iv);
  ...
  RNCryptor.Decrypt = function(b64str, password) {
    ...
    let cipher = crypto.createDecipheriv(_settings.algorithm, key, components.headers.iv);
    return Buffer.concat([cipher.update(components.cipher_text), cipher.final()]);
  };
