(function() {
  ...
  var cryptian = require('cryptian');

  var _settings = {};

  var _configure_settings = function(version) {
    var settings = {
      algorithm: cryptian.algorithm.Rijndael128,
      salt_length: 8,
      iv_length: 16,
      pbkdf2: {
        iterations: 10000,
    ...
    switch(version) {
      case 3:
        settings.mode = cryptian.mode.cbc;
        settings.options = 1;
  ...
  var _generate_iv = function (block_size) {
    var iv = crypto.randomBytes(_settings.iv_length);
  ...
  var _encrypt = function(plain_text, components, encryption_key, hmac_key) {
    ...
    var algorithm = _settings.algorithm();
    algorithm.setKey(encryption_key);

    var cipher = _settings.mode.Cipher(algorithm, components.headers.iv);
  ...
  RNCryptor.Decrypt = function(b64str, password) {
    ...
    var cipher = _settings.mode.Decipher(algorithm, components.headers.iv);
